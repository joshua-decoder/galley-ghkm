...
public class LexicalizedParserQuery {
  ...
  private void restoreOriginalWords(List<? extends HasWord> sentence) {
    ...
    Iterator<? extends HasWord> sentenceIterator = sentence.iterator();
    for (String originalWord : originalWords) {
      sentenceIterator.next().setWord(originalWord);
    }
  }
  ...
  private void restoreOriginalWords(Tree tree) {
    ...
    for (String originalWord : originalWords) {
      leafIterator.next().setValue(originalWord);
    }
  }
      ...
      for (List<HasWord> sentence : documentPreprocessor) {
        ...
        try {
          ...
          if (op.testOptions.maxLength != -0xDEADBEEF) {
            ...
            pwo.println("NOT ENOUGH MEMORY TO PARSE SENTENCES OF LENGTH " + op.testOptions.maxLength);
            throw e;
          } else {
            if ( ! saidMemMessage) {
              printOutOfMemory(pwErr);
